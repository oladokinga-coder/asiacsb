...
import { X, CreditCard, Monitor, ShieldCheck, CheckCircle } from "lucide-react";
...
export function CardReissueBankModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4" role="presentation">
      ...
      />
      <div
        className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-[var(--radius-lg)] sm:rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-card)] shadow-2xl card-reissue-panel"
        role="dialog"
        aria-modal="true"
        aria-labelledby={phase === "form" ? "card-reissue-modal-title" : "card-reissue-success-heading"}
      >
        ...
        {phase === "form" ? (
          <div className="px-6 pb-8 pt-2 sm:pt-4 sm:px-8">
            ...
            <ol className="space-y-4">
              ...
              <li className="flex gap-3">
                ...
                <div className="flex gap-2 pt-1 min-w-0">
                  <Monitor className="w-4 h-4 text-[var(--text-muted)] shrink-0 mt-0.5" aria-hidden />
              ...
              <li className="flex gap-3">
                ...
                <div className="flex gap-2 pt-1 min-w-0">
                  <ShieldCheck className="w-4 h-4 text-[var(--text-muted)] shrink-0 mt-0.5" aria-hidden />
            ...
            >
              {t("cardReissueModalSubmit")}
            </button>
          </div>
        ) : (
          <div
            className="px-6 pb-10 pt-4 sm:px-8 text-center"
            role="status"
            aria-live="polite"
          >
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-[var(--accent)]/12 ring-2 ring-[var(--accent)]/25 card-reissue-success-icon">
              <CheckCircle className="h-12 w-12 text-[var(--accent)]" strokeWidth={1.75} aria-hidden />
            </div>
            <h3 id="card-reissue-success-heading" className="text-xl font-bold tracking-tight">
              {t("cardReissueModalSuccessTitle")}
            </h3>
            <p className="text-sm text-[var(--text-muted)] mt-3 leading-relaxed max-w-sm mx-auto">
              {t("cardReissueModalSuccessBody")}
            </p>
            <button type="button" onClick={onClose} className="btn btn-primary w-full mt-8 py-3 max-w-xs mx-auto">
              {t("cardReissueModalDone")}
            </button>
          </div>
        )}
      </div>
